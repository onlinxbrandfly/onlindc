...
export default function CRMLeadTable({ leads, tasks, onOpen }) {
  ...
  return <div className="adminCard crmLeadsPanel"><div className="crmSectionHead"><div><h3>All leads</h3><p className="muted">The most useful opportunities appear first.</p></div><b>{leads.length} leads</b></div>
    <div className="crmLeadCards">{leads.map((lead) => { const contact = leadContact(lead); const call = crmCallLink(lead); const whatsapp = crmWhatsappLink({ lead }); return <article key={lead.id} className="crmLeadCard" onClick={() => onOpen(lead)}><div><span className="crmStage">{lead.stage || lead.status || "New"}</span><h3>{contact.businessName}</h3><p>{contact.contactName || "No contact name"}{contact.phone ? ` - ${contact.phone}` : ""}</p></div><div className="leadCardMeta"><b>{lead.priority_label}</b><span>{lead.next_action || "No next action"}</span></div><div className="rowActions" onClick={(event) => event.stopPropagation()}>{call && <a href={call}>Call</a>}{whatsapp && <a href={whatsapp} target="_blank" rel="noreferrer">WhatsApp</a>}<button onClick={() => onOpen(lead)}>Open</button></div></article>; })}</div>
